...
- **User-facing product text is localized** through each platform's resources (see `docs/recly.md` §7): English is the base language; the supported translations are listed in `localization/languages.json`. Additional language resources are generated with `python3 scripts/localize.py` and checked with `--check`. Never hardcode UI text in code.
