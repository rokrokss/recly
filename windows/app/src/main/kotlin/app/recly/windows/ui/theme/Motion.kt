 ...
 *
 * The numbers are the phone's and the Mac's (`android/.../ui/theme/Motion.kt`), because the promise
 * is the same promise on all three.
 */
object Motion {
    /** `ease-in-out 200ms`. */
 ...
 * ([Motion.BADGE_FADE_MS]) when the work overran that window.
 */
fun doneBadgeMs(workMs: Long): Long {
    val shown = workMs + processingHoldMs(workMs)
