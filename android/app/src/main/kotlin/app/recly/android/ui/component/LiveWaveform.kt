...
fun LiveWaveform(peaks: () -> List<Float>, modifier: Modifier = Modifier) {
    ...
    // The levels are the service's and change on its poller without Compose noticing; a state of
    // three hundred floats at 10 Hz would instead recompose everything that reads the model. This
    // counter is the whole of the state, and reading it is what asks for a new reading.
    ...
    // The tick is read *here*, in the composition, and the service is asked in the same breath: a
...
/** Tall enough to read as a level and short enough to leave the timer the screen. */
