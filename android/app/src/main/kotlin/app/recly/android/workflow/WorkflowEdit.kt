 ...
 * asynchronous, which "폴링 · 상태" recommends on a phone.
 */
...
/**
 * `2026-08-26T01:00:00.000Z` — the docs/01 stamp, three fraction digits and all. The core's own
 * formatter is internal to it, and `Instant.toString` drops a zero fraction.
 */
...
fun StepEdit.toStep(): Step = when (this) {
    ...
    is StepEdit.Transcribe -> Step.Transcribe(
        ...
        // and the form hides the field for the last kind, so carrying a leftover URL out would
        // fail validation for a field nobody can see.
