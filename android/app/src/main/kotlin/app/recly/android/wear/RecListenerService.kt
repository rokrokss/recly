...
class RecListenerService : WearableListenerService() {
     ...
     *
     * Nothing is remembered from [onChannelOpened]: Play Services creates a fresh instance of this
     * service for every event and destroys it a second later (`WearableLS` onCreate/onDestroy around
     * each `onChannelEvent`, Z Fold7 2026-09-04), so a map of open channels kept in a field was
     * empty by the time the input closed, and every part was dropped without a word. The staged
     * file is a pure function of the path, which is all this needs.
     */
    override fun onInputClosed(channel: ChannelClient.Channel, closeReason: Int, appErrorCode: Int) {
        val parsed = TransferPath.parse(channel.path) ?: return
        val file = stagingFile(parsed)
        // Never accepted here — a rejected path, or a receiveFile that failed and cleaned up.
        ...
        runBlocking {
            ...
            withContext(core.deps.io) {
                when (parsed) {
                    is TransferPath.PartFile -> acceptPart(core, channel.nodeId, parsed, file)
                    is TransferPath.Meta -> acceptMeta(core, channel.nodeId, parsed, file)
                }
            }
        }
    }
