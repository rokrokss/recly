...
Google Drive에 남고, 그다음 일(웹훅, 녹취, 요약)은 **내가** 만든 워크플로우가 **내** 키로 처리합니다.
...
- **파일과 웹훅이 인터페이스.** 녹음이 끝나면 서명된 웹훅을 호출해 n8n, Cloudflare Worker, 내 스크립트가
  ...
  등) 중 하나에 내 키를 넣어 씁니다. 요약은 유료 티어가 아니라 내 에이전트의 몫입니다.
...
| 요약 | 내 AI 에이전트(Claude Code, Codex 등) | 에이전트가 이미 접근할 수 있는 것. Recly는 관여하지 않습니다. |
...
Recly의 파이프라인은 일부러 녹취록에서 끝납니다. 텍스트 요약은 이미 쓰고 있는 AI 구독이 잘하는 일이라,
Recly는 과금되는 기능 대신 에이전트용 **스킬**을 함께 배포합니다.
[`skills/recly-notes`](skills/recly-notes/SKILL.md)는 내 Drive `recly/` 폴더에서 녹음의 녹취록을 읽어
회의록을 바로 옆에 씁니다.
...
그다음 이렇게 부탁하면 됩니다. *"Recly 폴더에서 어제 오후 3시 회의 요약해 줘"*, *"마지막 녹음의 액션
아이템 뽑아 줘"*. 회의록 양식이 마음에 안 들면 스킬 파일을 고치면 됩니다. 그게 이 구조의 요점입니다.
