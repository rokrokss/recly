...
- **이 기기에서 Drive 사용 중지**: 설정 → Google Drive → 연결된 행에서 선택합니다. 이 기기의 Google 인증 정보와 로그인 힌트를 지웁니다. 녹음, 작업 기록, API 키는 유지되며 Google의 접근 승인은 철회할 때까지 남습니다.
- **Google 접근 권한 철회**: 같은 연결 관리 화면에서 선택합니다. Google 접근 승인을 철회하고 이 기기의 Google 인증 정보, 작업 기록, Drive 캐시를 정리합니다. **녹음, Drive 파일, 워크플로우, 기기 기본값, API·웹훅 키, iPhone 전송 허용은 남습니다.** 녹음은 목록에서 별도로 삭제합니다.
  - 같은 Google 승인을 쓰는 다른 기기도 접근 권한을 잃을 수 있음을 확인 화면에 표시합니다. 이 기기의 대기 중인 처리는 정리됩니다. Google 권한 설정(<https://myaccount.google.com/permissions>) 링크는 미연결 상태에서도 제공합니다.
  - 철회 실패는 화면에 알리고 Google 권한 설정으로 안내합니다. 로컬 정리가 실패하면 연결 관리에서 재시도할 수 있습니다.
...
앱을 삭제하기 전에 워크플로우 화면의 시크릿 목록에서 저장한 API 키와 웹훅 서명 키를 각각 지우고, 설정 → Google Drive의 **Google 접근 권한 철회**로 Google 인증 정보와 이 기기의 작업 기록을 지우세요. 녹음도 지우려면 목록에서 개별 삭제하세요. iPhone에서 이후 외부 전송을 멈추려면 설정 → 개인정보 보호에서 허용을 철회하세요. 워크플로우·설정·데스크톱 데이터 폴더는 위 표에 따라 별도로 지워야 합니다. 이미 Drive·전사 업체·웹훅 운영자가 받은 데이터는 해당 서비스에서 따로 관리해야 합니다.
