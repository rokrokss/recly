...
- **연결 해제**: 설정 → Google Drive → 연결 해제에서 확인합니다. Google 접근 승인을 철회하고 이 기기의 Google 인증 정보, 작업 기록, Drive 캐시를 정리합니다. **녹음, Drive 파일, 워크플로우, 기기 기본값, API·웹훅 키, iPhone 전송 허용은 남습니다.** 녹음은 목록에서 별도로 삭제합니다.
  - 권한 철회는 같은 Google Cloud 프로젝트의 모든 클라이언트에서 이 Google 계정에 대한 Recly의 Drive 접근 승인을 제거합니다. 같은 계정을 쓰는 다른 기기에서도 Drive를 다시 사용하려면 Recly를 다시 연결해야 합니다. Google의 철회 반영에는 시간이 걸리며, 모든 기기의 화면이 즉시 미연결로 바뀌는 것은 아닙니다. 이 기기의 대기 중인 처리는 정리됩니다. Google 권한 철회에 실패했을 때만 설정에 Google 권한 설정(<https://myaccount.google.com/permissions>) 링크를 표시하며, 이 기기의 로컬 연결이 해제되어도 실패 기록이 남아 있으면 링크를 유지합니다. 철회 실패 기록이 없으면 링크는 표시하지 않습니다.
...
앱을 삭제하기 전에 워크플로우 화면의 시크릿 목록에서 저장한 API 키와 웹훅 서명 키를 각각 지우고, 설정 → Google Drive의 **연결 해제**로 Google 인증 정보와 이 기기의 작업 기록을 지우세요. 녹음도 지우려면 목록에서 개별 삭제하세요. iPhone에서 이후 외부 전송을 멈추려면 설정 → 개인정보 보호에서 허용을 철회하세요. 워크플로우·설정·데스크톱 데이터 폴더는 위 표에 따라 별도로 지워야 합니다. 이미 Drive·전사 업체·웹훅 운영자가 받은 데이터는 해당 서비스에서 따로 관리해야 합니다.
