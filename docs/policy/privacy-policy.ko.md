...
**App Store 지역.** iPhone과 iPad에서는 OpenAI 전사 기능의 사용 가능 여부를 결정하기 위해 Apple StoreKit으로 현재 App Store 국가 또는 지역을 조회합니다. 중국 본토 스토어에서는 이 기능을 비활성화하고, 지역을 확인할 수 없으면 해당 전사를 대기시킵니다. 이 StoreKit 조회에 녹음·녹취록·워크플로우 정의·타사 API 키를 전달하지 않으며 스토어 국가를 디스크에 저장하지 않습니다. 계정 및 스토어 서비스는 Apple이 관리합니다. 로컬 녹음과 Google Drive 업로드는 이 확인과 독립적으로 동작합니다.
...
- **연결 해제**: 설정 → Google Drive → 연결 해제에서 확인합니다. Google 접근 승인을 철회하고 이 기기의 Google 인증 정보, 완료 작업 기록, Drive 캐시를 정리합니다. 미완료 작업과 Drive 소유자 식별자는 기기에 보존하여 같은 계정으로 재연결하면 성공한 단계를 반복하지 않고 이어갑니다. 다른 계정의 작업은 대기 상태를 유지합니다. **녹음, Drive 파일, 워크플로우, 기기 기본값, API·웹훅 키, iPhone 전송 허용은 남습니다.** 녹음은 목록에서 별도로 삭제합니다.
...
| macOS | **앱 번들(`.app`)만 지워집니다** | ① `~/Library/Application Support/app.recly.mac/`(녹음, `rec.db`, `device.id`)를 직접 지우십시오. ② 키체인 접근에서 서비스 이름이 `app.recly.mac.secrets`인 항목(입력해 둔 API 키·웹훅 서명 키)과 `app.recly.drive.oauth` 항목(Google 토큰), 이전 Google 로그인 SDK의 `auth` 항목이 남아 있다면 함께 지우십시오. ③ **앱 설정이 `UserDefaults`에 남습니다** — 녹음 모드·동의 리마인더·언어·접근성 설정입니다. 이전 버전에서 아직 이관하지 않았다면 이메일 힌트(`app.recly.auth.lastAccount`)도 남아 있을 수 있습니다. 터미널에서 `defaults delete app.recly.mac`으로 함께 지웁니다 |
...
앱을 삭제하기 전에 워크플로우 화면의 시크릿 목록에서 저장한 API 키와 웹훅 서명 키를 각각 지우고, 설정 → Google Drive의 **연결 해제**로 Google 인증 정보를 지우세요. 미완료 작업 기록은 해당 녹음을 삭제할 때 함께 지워집니다. 녹음도 지우려면 목록에서 개별 삭제하세요. iPhone에서 이후 외부 전송을 멈추려면 설정 → 개인정보 보호에서 허용을 철회하세요. 워크플로우·설정·데스크톱 데이터 폴더는 위 표에 따라 별도로 지워야 합니다. 이미 Drive·전사 업체·웹훅 운영자가 받은 데이터는 해당 서비스에서 따로 관리해야 합니다.
