...
- **Stop using Drive on this device**: open Settings → Google Drive → the connected row. This signs out locally and removes this device's Google credentials (and the stored account email on Android). Recordings, job state and API keys remain. Google authorization remains until revoked.
- **Revoke Google access**: available in the same connection management screen. It revokes Google authorization and clears this device's Google credentials, job records and cached Drive references. **Recordings, Drive files, workflow definitions, the device default, API keys, webhook signing keys and iPhone transfer permissions remain.** Delete recordings separately from the recording list.
  - The confirmation explains that other devices using the same Google authorization may also lose access. Pending processing on this device is cleared. The screen links to Google permission settings (<https://myaccount.google.com/permissions>); this link is also available while disconnected.
  - **If revocation fails, the app says so** and keeps a route to Google permission settings. If local cleanup fails, connection management offers a retry.
