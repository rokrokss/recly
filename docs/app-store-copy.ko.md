...
공개 설명 첫 단락은 Google 연결 없이 로컬 녹음·저장·재생이 가능함을 밝힌다. Google 연결은 Drive 접근용이며, 현재 전사에는 연결이 필요하다는 점(원본을 올린 뒤 전사한다)도 명시한다. 녹음·사용자 Drive 저장·선택 전사(기기 안 또는 사용자가 고른 업체)가 설명의 중심이다. 앱 이름은 `Recly: Record for Your AI`로, 녹음과 전사 결과를 사용자가 선택한 AI에 활용하는 방향을 표현한다. 이름의 `AI`는 특정 업체를 지칭하지 않으며, 공개 설명에는 선택 기능의 설정·외부 계정·요금 조건을 명시한다.
...
현재 영어 문안은 이름 25/30자, 부제 28/30자, 프로모션 118/170자, 설명 1088/4,000자, 키워드 81/100바이트다. 이름에는 일반적인 `AI` 표현을 사용하며, 공개 문안에는 ChatGPT/OpenAI/GPT 참조를 넣지 않았다.
