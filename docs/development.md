...
현재 배포의 표시 버전은 모든 플랫폼에서 `0.1.0`이다. Apple 앱·내장 Watch·위젯의 빌드는 `18`,
Android는 `23`, Wear OS는 `1,000,023`이다. Windows 앱 표시 버전도 `0.1.0`으로 유지하고,
업그레이드 구분을 위해 MSI의 세 번째 버전 필드만 올려 설치 버전은 `0.1.17`으로 설정한다.
