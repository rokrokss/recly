...
삭제 버튼은 비활성화**하고, 행에 "삭제하려면 먼저 다른 워크플로우를 선택하세요"를 표시한다. 삭제 처리에서도
최신 기기 포인터를 확인한다. 하나뿐이면 새 워크플로우를 만든 뒤 선택해야 기존 것을 삭제할 수 있다.
가져오기로 문서를 교체해 선택한 정의가 사라지면 포인터를 비우고 목록·녹음 화면에 "워크플로우를 선택하세요"를 띄운다.

워크플로우 목록 상단에는 개수 숫자를 표시하지 않는다. 생성 버튼은 `+ 새 워크플로우`, `+ 시크릿 추가`이며,
기호와 문구를 하나의 터치 대상으로 제공한다. 모바일 행의 삭제 버튼은 오른쪽에, 시크릿 추가·폼 작업도 오른쪽에
배치한다. 데스크톱은 기존 가로 배치를 유지하되 삭제 제한과 생성 버튼 표현은 동일하게 적용한다.
...
2. **목록 = 원장(ledger)**: 행 = `시각(모노) · 제목 · 길이 · 상태 코드 · 진행`. 시각 열은 4셸 모두 언어와 무관하게
   ...
   **Android·iPhone 상세의 재생 영역은 본문 아래에 고정한다**(2026-09-10). 녹취 본문만 스크롤하고, 파형 아래에
   경과/총 길이를 왼쪽, 재생/일시정지 버튼을 오른쪽에 둔다. Android의 공통 터치 영역은 최소 48×48dp이며 본문을
   덮지 않는다. **macOS·Windows는 기존 상단 재생 영역과 데스크톱 창·분할 배치를 유지한다.**
   4셸 모두 전사 본문 선택·전체 복사·본문/화자 검색·발화 시각 이동을 제공한다. 검색 중에도 전체 복사는
   원문 전체와 시각을 복사한다. 시각 이동은 녹음 중·오디오 확인/다운로드 중에는 비활성이며, 실제 보유한
   오디오 범위 밖의 시각도 비활성이다. 검색·복사는 오디오 유무와 관계없이 제공한다.
   ...
   재생·탐색을 차단하고, 녹음 시작 실패 시에도 차단을 해제한다. 다른 녹음의 시작·종료도 열린 상세에 반영한다.
   ...
   파일은 다시 내려받지 않는다. 빈 전사에는 녹음 재생 안내를 제공한다. 실제 재생 실패도 화면에 표시하고 다음
   ...
   녹음 목록의 삭제는 동작 영역 우측 끝에 고정하고, 열기·다시 시도·상세 등 나머지 동작은 왼쪽 공간에서
   줄바꿈한다. 모바일 목록과 데스크톱 메뉴·트레이의 펼친 목록에도 같은 규칙을 적용한다.
...
3. **워크플로우 편집기 = 노드 그래프**: 트리거(기기) → 단계 노드들 → 끝. 노드는 사각, 커넥터는 직선, 선택 노드에
   ...
   동작이 없는 정보이며, `사용`·키 추가·삭제는 독립 동작이다. 수정은 기존 미저장 변경 보호 절차를 따른다.
   ...
   중복되는 편집 헤더를 접어 저장 영역을 확보한다. 전사 검색 입력 중에는 상세 헤더·재생 영역을 접고, 키보드를
   닫으면 다시 표시한다. Apple의 긴 버튼 문구는 화면 폭 안에서 최대 세 줄로 재배치한다.
   Apple Watch는 주 화면을 스크롤하고 상태 문구를 줄바꿈하며, Wear의 긴 워크플로우명·전송 상태도 여러 줄을 허용한다.
   4셸 모두 미저장 워크플로우·키 입력을 버리는 취소나 편집 대상 교체를 확인한다. 변경 없는 편집에는 확인을 추가하지
   않는다. 다른 키 입력 폼을 여는 경우도 폐기 확인을 거치며, 같은 키 입력을 다시 선택하면 현재 입력을 유지한다.
...
- **Mac 팝오버 크기**: 메뉴바와 Dock을 제외한 현재 화면 안에 들어오도록 높이를 제한한다. 설정·원장만 스크롤하고
  하단의 설정/뒤로·종료는 항상 표시한다. 설정에서 돌아오면 원장 크기로 줄어들며, 화면 구성이 바뀌면 위치와 높이를 다시 맞춘다.
